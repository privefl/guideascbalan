...
marathons <- tibble::tribble(
  ~ville,  ~date,      ~chrono,
  # "Vercors", "27/06/26", "3:26:00"
) %>% 
  mutate(course = "Marathon", dist = 42.2) %>% 
  print()
...
semis <- tibble::tribble(
  ...
  "Bourg-en-Bresse", "01/03/26", "1:23:48"
) %>% 
...
all <- bind_rows(tests_VMA, marathons, semis, `10K`, DHL5K) %>% 
  mutate(
    date    = dmy(date),
    course  = factor(course, c("Test VMA", "5K DHL Aarhus", "10 km", "Semi-marathon", "Marathon")),
    vitesse = ifelse(!is.na(vitesse), vitesse, dist / parse_chrono(chrono))
  ) %>% 
  print()
```
...
ggplot(all, aes(date, vitesse, color = course, label = ville)) +
  ...
  scale_x_date(
    breaks = dmy(paste0("01/09/", 22:(year(today()) - 2000))),
    date_labels = "%d/%m/%y", 
    date_minor_breaks = "1 month",
    limits = c(dmy("01/09/22"), today()),
    expand = expansion(mult = c(0.02, 0.02)),  # no expansion on left, small on right
    sec.axis = dup_axis(breaks = as.Date("1993-03-21") + years(30:100),
                        labels = paste(30:100, "ans"), name = NULL)
  ) +
  ...
  labs(x = NULL, y = "Vitesse (km/h)", color = NULL)
```
