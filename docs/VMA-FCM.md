...
La VMA est la vitesse à laquelle un coureur utilise son maximum d'oxygène (VO2 max) pendant l'effort.
Elle est souvent considérée comme un bon indicateur de la performance aérobie d'un coureur.
La VMA est généralement mesurée en km/h et peut être déterminée par des tests spécifiques.

### Pourquoi est-elle importante ? {-}

- **Détermination des allures d'entraînement** :
La VMA permet de définir des intensités d'entraînement précises pour le développement de l'endurance, des séances de seuil et de fractionnés.
...
Pour calculer la VMA en fonction de la durée du test, il suffit d'appliquer la formule suivante : `VMA = (min + sec / 60) / 2 + 7,5`. Par exemple, lors du test VMA du 08/10/24, j'ai tenu 19 minutes et 32 secondes, ce qui me donne une VMA de 17,27.


## Qu'est-ce que la FCM ? {-}

La Fréquence Cardiaque Maximale (FCM) est le nombre maximum de battements que votre cœur peut atteindre en une minute lors d'un effort maximal.
...
La FCM est utilisée pour définir des zones de fréquence cardiaque qui guident les coureurs
dans l'intensité de leurs entraînements.
